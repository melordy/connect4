...
*/
public class minimax_SWEGGERZ extends AIModule
{
	private int player1;
	private int player2;
	...
	private int evaluationFunction(GameStateModule game)
	{
		if(game.isGameOver())
		{
		...
		// OneWin is a possible four-in-a-row winning combination
		for(int[][] oneWin : this.winningFour)
		{
	...
	private int minimax(GameStateModule game, int depth, int curr, boolean maximizingPlayer)
	{
		if(game.isGameOver() || terminate || depth == 0)
		{
			// run eval function if end of the game
			// or reached the bottom of minimax tree
		...
		if(maximizingPlayer)
		{
			...
			for(int i = 0; i < game.getWidth(); i++)
			{
				...
				{
					game.makeMove(i);
					v = Math.max(v, minimax(game, depth-1, curr, !maximizingPlayer));
					game.unMakeMove();
				}
			}
			return v;
		...
		else // minimizing player
		{
			...
			for(int i = 0; i < game.getWidth(); i++)
			{
				...
				{
					game.makeMove(i);
					v = Math.min(v, minimax(game, depth-1, curr, !maximizingPlayer));
					game.unMakeMove();
				}	
			}
			return v;
	...
	public void getNextMove(final GameStateModule game)
	{
		...
		// depth of minimax tree (# of moves ahead from chosen move)
		int depth = 4;
		...
		for(int i = 0; i < game.getWidth(); i++)
		{
			if(game.canMakeMove(i))
			{
				...
				int min = minimax(game, depth, i, false);
				game.unMakeMove(); // go back up tree
				if(min > max)
				{
					max = min;
					chosenMove = i;
